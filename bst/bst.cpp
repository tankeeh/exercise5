...
/** FUNZIONI NODO **/
    ...
    typename BST<Data>::BSTNode const* BST<Data>::BSTNode::Find(const Data& key)const{
        ...
        while(temp != nullptr && temp->Element() != key){
            ...
            else if(key > temp->Element()) temp = temp->Right();
        }
        return temp;
    }
    ...
    typename BST<Data>::BSTNode* BST<Data>::BSTNode::PredecessorParent(const Data& key) const {
        ...
        else if(currnode->HasLeftChild()){
            if( !currnode->Left()->HasRightChild())
                return const_cast<BSTNode*>(currnode); //forse dovrebbe tornare temp, non currnode(?)
            else {
    ...
    void BST<Data>::Remove(BSTNode* currnode,BSTNode* father) noexcept {
        ...
        if (currnode->IsLeaf()) {
            ...
            if(father == nullptr){
                ...
                this->Node = currnode->Left();
                temp->sx = nullptr;
                delete temp;
            }
            else if (currnode->Left()->IsLeaf()) {
    ...
    //FUNZIONE DI RIMOZIONE DEL MINIMO (CUSTOM)
    template<typename Data>
